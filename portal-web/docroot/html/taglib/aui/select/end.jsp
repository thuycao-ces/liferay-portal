...
<div class="<%= controlGroupCssClass %>">
	...
	<select class="<%= fieldCss %>" <%= disabled ? "disabled" : StringPool.BLANK %> id="<%= namespace + id %>" <%= multiple ? "multiple" : StringPool.BLANK %> name="<%= namespace + name %>" <%= Validator.isNotNull(onChange) ? "onChange=\"" + onChange + "\"" : StringPool.BLANK %><%= StringPool.SPACE %><%= Validator.isNotNull(onClick) ? "onClick=\"" + onClick + "\"" : StringPool.BLANK %><%= StringPool.SPACE %><%= Validator.isNotNull(title) ? "title=\"" + LanguageUtil.get(resourceBundle, title) + "\"" : StringPool.BLANK %><%= StringPool.SPACE %><%= AUIUtil.buildData(data) %><%= StringPool.SPACE %><%= InlineUtil.buildDynamicAttributes(dynamicAttributes) %>>
