...
<span class="taglib-workflow-status">
	...
	<span class='<%= showIcon ? "icon-file-alt workflow-status" : "workflow-status" %>'>
		...
		<strong class="label label-<%= WorkflowConstants.getStatusStyle(status) %> status text-uppercase workflow-status-<%= WorkflowConstants.getStatusLabel(status) %><%= StringPool.SPACE %><%= WorkflowConstants.getStatusCssClass(status) %> workflow-value">
